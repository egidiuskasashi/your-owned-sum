 ...
 *  @notice together with the sender address who sent the transaction, who actually becames the sum's owner */
contract sumandstore {
    ...
    /** @notice sum is a data structure modelling a single sum */
    struct sum {
        uint add1;
        uint add2;
        uint sumResult;
    ...
    /** @notice  owner owned sums is store and updated */
    mapping (address => uint) sumsPerOwnerCount;

    /** @notice mapping to resolve what is the sumId owned by a particular address */
